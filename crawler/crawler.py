...
get_url = lambda \
        x: "https://www.zhipin.com/c101010100/?query=%E6%B5%8B%E8%AF%95%E5%B7%A5%E7%A8%8B%E5%B8%88&page={}&ka=page-{}".format(
    x, x)
...
def crawler():
    for i in range(1, 2):
        try:
            response = requests.get(url=boss_url, headers=headers, proxies=proxies)
            html = response.text
            print(html)
            html = etree.HTML(html)
            print("正在爬取第{}页数据".format(i))
            job_name_list = html.xpath("//*[@class='info-primary']//h3//a//div[@class='job-title']/text()")[1]
...
# response = requests.get(url=checkUrl, headers=headers, proxies=proxies)
